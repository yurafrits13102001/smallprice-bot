...
# Browser (Puppeteer) web-scraper: these marketplaces serve a challenge/block page
# without og:image to plain-HTTP clients, so a real browser is required (Cheerio got
# 0/3318). The run is given extra memory so Apify can fit more concurrent browsers —
# concurrency, not the per-page time, was the bottleneck (it sat at 1-2 on default RAM).
_APIFY_IMG_PAGEFUNC = (
    "async function pageFunction(context) {\n"
    "    await new Promise(r => setTimeout(r, 3000));\n"
...
# More RAM → Apify fits more concurrent Chrome instances → much higher throughput.
# 8 GB is widely allowed; raise toward 16384 if the plan permits, lower if the run is
# rejected for exceeding the per-run memory cap.
_APIFY_IMG_MEMORY_MB = 8192
...
def _scrape_images_apify_sync(urls: list[str], apify_token: str) -> dict[str, str | None]:
    ...
    try:
        ...
        run = client.actor("apify/web-scraper").call(
            run_input=run_input, memory_mbytes=_APIFY_IMG_MEMORY_MB
        )
